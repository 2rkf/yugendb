...
describe("PostgreSQL integration", () => {
  it("runs storage checks only when YUGENDB_POSTGRES_URL is set", async () => {
    const url = process.env.YUGENDB_POSTGRES_URL;
    ...
    const db = await createStore({ driver: postgres(url), namespace });
