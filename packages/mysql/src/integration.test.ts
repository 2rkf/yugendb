...
describe("MySQL integration", () => {
  it("runs storage checks only when YUGENDB_MYSQL_URL is set", async () => {
    const url = process.env.YUGENDB_MYSQL_URL;
    ...
    const db = await createStore({ driver: mysql(url), namespace });
